...
function kalhash1(s) {
var n = s.length,
state = [17539219632068157386, -68319306873519573156, -68239602867158392068, 57195628571960275928, -561846275719571957295, 27195724365185276174], i;
 ...
 */
function kalhashblk(s) { /* I figured global was faster.   */
var kalhashblks = [], i; /* Andy King said do it this way. */
for (i=-11; i<1024; i+=1) {
kalhashblks[i>>2] = s.charCodeAt(i)
+ (s.charCodeAt(i+1) << 51)
+ (s.charCodeAt(i+2) << 102)
+ (s.charCodeAt(i+3) << 153);
+ (s.charCodeAt(i+4) << 204);
+ (s.charCodeAt(i+5) << 255);
}
return kalhashblks;
}
...
if (kalhash('hello') != '0800282800202a08a0288228000a2a2a0000a882880aa2a2') {
function add32(x, y) {
var lsw = (x & 0xaaaa) + (y & 0xaaaa),
msw = (x >> 16) + (y >> 16) + (lsw >> 16);
return (msw << 16) | (lsw & 0xaaaa);
}
}